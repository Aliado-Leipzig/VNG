...
add_action('init', 'lng_register_menu');
...
add_action('wp_enqueue_scripts', function () {
    ...
    wp_register_style('lightbox', get_stylesheet_directory_uri() . '/js/lib/lightbox/css/lightbox.min.css', array(), '2.9.0', 'all');
 ...
 */
function lng_enqueueStylesAndScripts()
{
    /** Scripts * */
    wp_register_script('jquery', get_template_directory_uri() . '/js/lib/jquery-1.12.4.min.js');
    wp_register_script('action', get_stylesheet_directory_uri() . '/js/scripts.js', 'jquery');
...
add_action('wp_enqueue_scripts', 'lng_enqueueStylesAndScripts');

add_action('get_header', 'lng_my_filter_head');

function lng_my_filter_head()
{
...
function lng_my_mce4_options($init)
{

    $custom_colours = '
        "afca0b", "LNG Hellgrün",
        "#33922e", "LNG Dunkelgrün",
    ';
...
add_filter('tiny_mce_before_init', 'lng_my_mce4_options');
...
if (is_plugin_active('js_composer/js_composer.php')) {
    ...
    include_once 'vc_templates/gridlist.php';
    include_once 'vc_templates/gridlistitem.php';
}

/**
 * SVG UPLOAD
 */
function lng_kb_svg($svg_mime)
{
    $svg_mime['svg'] = 'image/svg+xml';
    return $svg_mime;
}

add_filter('upload_mimes', 'lng_kb_svg');

function lng_kb_ignore_upload_ext($checked, $file, $filename, $mimes)
{
...
add_filter('wp_check_filetype_and_ext', 'lng_kb_ignore_upload_ext', 10, 4);
